...
def distributed_init(args):
    ...
    global _LOCAL_RANK, _LOCAL_WORLD_SIZE
    _LOCAL_RANK, _LOCAL_WORLD_SIZE = local_rank, local_world_size
    ...
    local_ranks = torch.empty(world_size, dtype=torch.long, device="cuda")
    local_rank_tensor = torch.tensor([local_rank], dtype=torch.long, device="cuda")
    dist.all_gather_into_tensor(local_ranks, local_rank_tensor)

    local_world_sizes = torch.empty(world_size, dtype=torch.long, device="cuda")
    local_world_size_tensor = torch.tensor([local_world_size], dtype=torch.long, device="cuda")
    dist.all_gather_into_tensor(local_world_sizes, local_world_size_tensor)

    local_ranks = local_ranks.tolist()
    local_world_sizes = local_world_sizes.tolist()

    global _INTRA_NODE_PROCESS_GROUP, _INTER_NODE_PROCESS_GROUP
    _INTRA_NODE_PROCESS_GROUP = None
    _INTER_NODE_PROCESS_GROUP = None
    ...
    if min(local_world_sizes) == max(local_world_sizes):
        for i in range(local_world_size):
            group = dist.new_group(list(range(i, world_size, local_world_size)))
            if i == local_rank:
                _INTER_NODE_PROCESS_GROUP = group
