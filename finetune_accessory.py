...
from data import DataNoReportException, ItemProcessor, MyDataset, ImageParquetDataset
from imgproc import generate_crop_size_list, to_rgb_if_rgba, var_center_crop, center_crop
...
class T2IItemProcessor(ItemProcessor):
    ...
    def ta_tok_processor(self, data_item):
        if 'input_image' in data_item and 'prompt' in data_item:
            image = Image.open(data_item["input_image"])
        elif 'image' in data_item:
            # random pick an image if it is a list
            if isinstance(data_item['image'], list):
                data_item['image'] = random.choice(data_item['image'])
            if isinstance(data_item['image'], bytes):
                image = Image.open(io.BytesIO(data_item['image'])).convert('RGB')
            elif isinstance(data_item['image'], dict) and 'bytes' in data_item['image']:
                image = Image.open(io.BytesIO(data_item['image']['bytes'])).convert('RGB')
            else:
                raise ValueError("No image found in data_item")
            
        text = "You are an assistant designed to generate superior images with the highest degree of image alignment based on the SigLIP2 features of an original image. <Prompt Start> "
...
def main(args):
    ...
    model = models.__dict__[args.model](
        in_channels=16,
        cond_in_channels=1152, # 1152 is the hidden_dim of siglip2
    ...
    vae = AutoencoderKL.from_pretrained(args.vae_path, subfolder="vae", torch_dtype=torch.bfloat16).to(device)

    ta_tok = TextAlignedTokenizer.from_checkpoint(
        args.ta_tok_path, load_teacher=False, input_type='rec'
    ).to(device=device, dtype=torch.bfloat16)
    ta_tok.eval()
    from tok.utils import ScalingLayer
    ta_tok.scale_layer = ScalingLayer(mean=[0., 0., 0.], std=[1.0, 1.0, 1.0]).to(device=device, dtype=torch.bfloat16)   
    ...
    for train_res in [1024]:
        ...
        image_transform = transforms.Compose(
            [
                ...
                transforms.Lambda(functools.partial(center_crop, crop_size=[train_res, train_res])),
                transforms.ToTensor(),
        ...
        dataset = ImageParquetDataset(args.data_path, T2IItemProcessor(image_transform))
        num_samples = global_bsz * args.max_steps
        
        loader = DataLoader(
            dataset,
            batch_size=local_bsz,
            # sampler=sampler, # no sampler for iterative dataset
    ...
    # setup global cap for ta_tok
    global_cap_feats, global_cap_mask = None, None
    ...
    for step in range(resume_step, args.max_steps):
        ...
        for train_res, data_pack in data_collection.items():
            ...
            for data_item, group_name in zip(data_items, group_names):
                if group_name == 'ta_tok':
                    x_, caps_, cond_ = t2i_item_processor.ta_tok_processor(data_item)
                    x.append(x_)
                    caps.append(caps_)
                    cond.append(cond_)
            ...
            with torch.no_grad():
                ...
                for i, img in enumerate(x):
                    ...
                    x[i] = x[i].float()
                    cond_ins = []
                    for cond_img in cond[i]:
                        cond_in = ta_tok(cond_img[None].bfloat16())['encoded']
                        B, L, D = cond_in.shape
                        H = W = int(L**0.5)
                        cond_in = cond_in.permute(0, 2, 1).reshape(B, D, H, W)
                        # resize to x[i].shape
                        cond_in = F.interpolate(cond_in, size=x[i].shape[-2:], mode='bilinear')
                        cond_ins.append(cond_in[0].float())
                    cond_ins_all.append(cond_ins)
                cond = cond_ins_all
        ...
        # Save DiT checkpoint:
        if (step + 1) % args.ckpt_every == 0 or (step + 1) == args.max_steps or (step + 1) % 200 == 0:
            ...
            if dist.get_rank() == 0:
                ...
                if hasattr(args, "keep_last_n") and args.keep_last_n > 0:
                    # list subdirs sorted by step number
                    all_ckpts = sorted(
                        [d for d in os.listdir(checkpoint_dir) if d.isdigit()],
                        key=lambda x: int(x)
                    )
                    if len(all_ckpts) > args.keep_last_n:
                        to_delete = all_ckpts[: len(all_ckpts) - args.keep_last_n]
                        for d in to_delete:
                            rm_path = os.path.join(checkpoint_dir, d)
                            import shutil
                            shutil.rmtree(rm_path, ignore_errors=True)
                            logger.info(f"Removed old checkpoint {rm_path}")
            dist.barrier()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vae_path", type=str, default='black-forest-labs/FLUX.1-dev')
    parser.add_argument("--text_encoder_path", type=str, default='google/gemma-2-2b')
    parser.add_argument("--ta_tok_path", type=str, default='')
    parser.add_argument(
